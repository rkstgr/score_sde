...
def init_model(rng, config):
    ...
    if config.data.dataset == "MTG":
        time_bins = int(np.ceil(int(config.data.sampling_rate * config.data.duration) / config.data.hop_length))
        input_shape = (jax.local_device_count(), config.data.n_fft // 2, time_bins, config.data.num_channels)
    else:
        input_shape = (
